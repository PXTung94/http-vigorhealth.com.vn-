...
if($tion=="save")
{
	...
	foreach ($page_name_arr as  $page_name){
		...
		if(($page_name)&&($page_name_en)){
			...
			$sql_qr = mysql_query("select page_name from tbl_pagename_ctrinh where (page_name_no='$page_name_no')");
			...
			$sql_qr = mysql_query("select page_name from tbl_pagename_ctrinh where (page_name_en_no='$page_name_en_no')");
			$sodong=@mysql_num_rows($sql_qr);$i=0;
			while($sodong){
				$i++;
			...
			$query = mysql_query("insert into tbl_pagename_ctrinh(page_name,page_name_no,page_of,page_name_en,page_name_en_no,page_parent) values 
		...
		echo "<script>location.href='add_page_ctrinh.php?page_parent=$page_parent';</script>";
...
/////////////
$addcat=<<<EOF
	<h1>Nhập Danh mục Giới thiệu</h1>
