...
if($com=="delete"){
	for($i=1;$i<=$socheck;$i++){
		$b="check".$i;
		$id=$$b;
		if($id) $sql_del = mysql_query("delete from tbl_goikham where  (lienketid ='$id')");
	}
}
if($com=="sort"){
	...
	for($i=1;$i<=$socheck;$i++){
		...
		if($sort_v){
		  ...
		  $sql_qr = mysql_query("update tbl_goikham set sapxep='$sort_v' where (lienketid ='$lienketid1')");
...
if ($del){
	...
	$sql_del = mysql_query("delete from tbl_goikham where (lienketid ='$del')");

}

if ($ina){

	$sql_qr = mysql_query("update tbl_goikham set hienthi=0 where lienketid ='$ina'");

}

if ($a){

	$sql_qr = mysql_query("update tbl_goikham set hienthi=1 where lienketid ='$a'");

}

echo "<h1>Danh sách Gói khám</h1>";

echo "<form action='mana_goikham.php' method='post' name='mainform' id='mainform'>

          <input name='com' type='hidden' value=''>";

$sql_level_1 = mysql_query("select  * from tbl_goikham order by sapxep,lienketid");
...
while($rows = mysql_fetch_array($sql_level_1)){
	...
	$a_gioithieu =outputdata($rows[a_gioithieu]);$a_gioithieu_en =outputdata($rows[a_gioithieu_en]);
		if ($hienthi==1){ 
		$ac="<a href='mana_goikham.php?ina=$lienketid'  class='green'>Hiển thị >>> Ẩn</a>";
		}else{ 
			$ac="<a href='mana_goikham.php?a=$lienketid'  class='gray'>Ẩn >>> Hiển thị</a>";
		}
		echo "<table>
<tr>
<th width='2%'>Check</th>
...
<td><ul>
	<li>Tên gói khám EN<right>$s_text_en</right></li>
	...
	<li>Giới thiệu VN<right>$a_gioithieu</right></li><li>Giới thiệu EN<right>$a_gioithieu_en</right></li>
	<li>Chức năng<right><a href='edit_goikham.php?edit=$lienketid' class='orange'>Sửa</a><a href='mana_goikham.php?del=$lienketid' class='orange' onclick=\"return confirm('Are you sure to del it ?')\">Xóa</a>$ac</right></li>
</ul></td>
</tr>
</table><br>";
			}
