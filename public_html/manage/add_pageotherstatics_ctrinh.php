...
<body>
<?
if ($timkiem_trang){ $_SESSION['ses_nhom_id_search']=$nhom_id_search; }
$nhom_id_search=$_SESSION['ses_nhom_id_search'];$okind=$nhom_id_search;
?>
<form name="searchproduct" method="post" action="add_pageotherstatics_ctrinh.php?timkiem_trang=1">
<h1>Nhập Nội dung Giới thiệu</h1>
<ul>
	<li>Chọn Danh mục >>><right><select name="nhom_id_search"><option value='0'>Chọn Danh mục</option>
   	<?
    $sql_level_cata = mysql_query("select  * from tbl_pagename_ctrinh where page_parent=0 order by page_sort,page_id");
    while($rowscata = mysql_fetch_array($sql_level_cata)){
        $pageid  = $rowscata[page_id];
        $page_name = trim($rowscata[page_name]);
        $sql_level_cata_1 = mysql_query("select  * from tbl_pagename_ctrinh where page_parent='$pageid' order by page_sort,page_id");
...
if($updatesubmit){
	...
	if ($thanhcong){
		...
		if($updatenow){
			$qr="update tbl_otherpage_ctrinh set content='$content',content_en='$content_en',meta_key='$meta_key',page_file='$pic_name',title_file='$title_file'
			,title_file_en='$title_file_en' where id='$id_page'";
			$qr_update=mysql_query($qr);
		}else{
			$qr_insert=mysql_query("insert tbl_otherpage_ctrinh(content,content_en,okind,meta_key,page_file,title_file,title_file_en) 
			values ('$content','$content_en','$okind','$meta_key','$pic_name','$title_file','$title_file_en')");
			$qr=mysql_query("select * from tbl_otherpage_ctrinh where okind='$okind' order by id desc limit 0,1");
		...
		for ($i=0;$i<11;$i++){
			...
			if($pic_namex){
				if(!(@is_numeric($pic_namex))){
					$del_pic=mysql_query("delete from tbl_pic_otherpage_ctrinh where n_vitri='$k' and n_id='$id_page'");
					$in_pic=mysql_query("insert into tbl_pic_otherpage_ctrinh(n_id,n_vitri,p_pic,kind) VALUES('$id_page','$k','$pic_namex','$okind')");
				}
			}
		}	
		echo "<script>
				alert('Đã cập nhật xong !');
				location.href='add_pageotherstatics_ctrinh.php';
			</script>";
		exit();
	}
}

if($delit) $dele = mysql_query("delete from  tbl_pic_otherpage_ctrinh where kind='$okind' and id='$delit'");

$qr=mysql_query("select * from tbl_otherpage_ctrinh where okind='$okind'");
...
?>
<br />
<form  action="add_pageotherstatics_ctrinh.php?updatenow=<?=$updatenow?>&id_page=<?=$id_page?>&updatesubmit=1" method="post" enctype="multipart/form-data" name="formuploaddata">
<h2><? 
	$sql_qr=mysql_query("select * from tbl_pagename_ctrinh where page_id='$okind'");
	$page_parent=@mysql_result($sql_qr,0,'page_parent');
	if($page_parent){
			$sql_qr_1=mysql_query("select * from tbl_pagename_ctrinh where page_id='$page_parent'");
...
<ul>
    ...
    for ($i=0;$i<10;$i++){
        ...
        $str_xoa="";
        $str_hinh="";
        $sql_picogiua = mysql_query("select * from  tbl_pic_otherpage_ctrinh where (n_id=$id_page) and (n_vitri=$k) and (kind='$okind') order by n_vitri");
        if (@mysql_num_rows($sql_picogiua)){
            $id=mysql_result($sql_picogiua,0,'id');
            $p_pic=mysql_result($sql_picogiua,0,'p_pic');
            $str_xoa="<a href='add_pageotherstatics_ctrinh.php?delit=$id' class='blue'>Xóa</a>";
