...
if($com=="delete"){
	for($i=1;$i<=$socheck;$i++){
		$b="check".$i;
		$id=$$b;
		if($id) $sql_del = mysql_query("delete from tbl_banner where  (lienketid ='$id')");
	}
}
if($com=="sort"){
	...
	for($i=1;$i<=$socheck;$i++){
		...
		if($sort_v){
		  ...
		  $sql_qr = mysql_query("update tbl_banner set sapxep='$sort_v' where (lienketid ='$lienketid1')");
...
if ($del){
	...
	$sql_del = mysql_query("delete from tbl_banner where (lienketid ='$del')");

}

if ($ina){

	$sql_qr = mysql_query("update tbl_banner set hienthi=0 where lienketid ='$ina'");

}

if ($a){

	$sql_qr = mysql_query("update tbl_banner set hienthi=1 where lienketid ='$a'");

}

echo "<h1>Banner Left</h1>";

echo "<form action='mana_partner.php' method='post' name='mainform' id='mainform'>

          <input name='com' type='hidden' value=''>";

$sql_level_1 = mysql_query("select  * from tbl_banner order by sapxep,lienketid");
...
while($rows = mysql_fetch_array($sql_level_1)){
	...
	if($type=="swf"){
		...
		$str_pic="<a href='../multidata/$s_pic' target='_blank'><img src='../multidata/$s_pic' width='350' border='0'></a>";
	}
	$s_text =outputdata($rows[s_text]);$s_text_en =outputdata($rows[s_text_en]);$s_link =outputdata($rows[s_link]);
		if ($hienthi==1){ 
		$ac="<a href='mana_partner.php?ina=$lienketid'  class='green'>Hiển thị >>> Ẩn</a>";
		}else{ 
			$ac="<a href='mana_partner.php?a=$lienketid'  class='gray'>Ẩn >>> Hiển thị</a>";
		}
		echo "<table>
<tr>
<th width='2%'>Check</th>
...
<td><ul>
	<li>Hình / Flash<right>$str_pic</right></li>
	<li>Liên kết<right><a href='$s_link' target='_blank'>$s_link</a></right></li>
	<li>Tiêu đề EN<right>$s_text_en</right></li>
	<li>Chức năng<right><a href='edit_partner.php?edit=$lienketid' class='orange'>Sửa</a><a href='mana_partner.php?del=$lienketid' class='orange' onclick=\"return confirm('Are you sure to del it ?')\">Xóa</a>$ac</right></li>
</ul></td>
</tr>
</table><br>";
			}
