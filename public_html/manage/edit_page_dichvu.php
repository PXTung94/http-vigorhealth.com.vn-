...
<?
if ($edit){
	$sql_qr=mysql_query("select * from tbl_pagename_dichvu where page_id='$edit'");
			$page_id=$edit;
...
if($submit){
			...
			$sql_qr = mysql_query("select page_name from tbl_pagename_dichvu where (page_name_no='$page_name_no') and (page_id <>'$page_id')");
			...
			$sql_qr = mysql_query("select page_name from tbl_pagename_dichvu where (page_name_en_no='$page_name_en_no') and (page_id <>'$page_id')");
			$sodong=@mysql_num_rows($sql_qr);$i=0;
			while($sodong){
				$i++;
			...
			$qr=mysql_query("update tbl_pagename_dichvu set page_name='$page_name',page_name_no='$page_name_no' ,page_name_en='$page_name_en',
page_name_en_no='$page_name_en_no' where page_id='$page_id'");
			echo "<script>
					alert('Đã sửa xong !');
					location.href='mana_page_dichvu.php?pz=$pz';
				</script>";
		exit();
}
$addform=<<<EOF
<h1>Sửa Danh mục Dịch vụ</h1>
