...
<?php
	...
	if ($bs){	
		...
		if($value_search){
			...
			$str_action.=" ((a_name like '%$value_search_no%')  or (a_name_en like '%$value_search_no%')) ";

		}
		if ($kind_search){
			 if($str_action) $str_action.=" and "; else $str_action.=" where ";
			 $str_action.= " ($kind_search) ";

		}
		
		if ($catalogid_search){
			if($str_action) $str_action.=" and "; else $str_action.=" where ";
			$str_action.=" (c_id ='$catalogid_search')";
		}
...
<?php
	if($com=="check"){	$tcheck="checked";
		...
		for($i=1;$i<=$socheck;$i++){
			...
			$id=$$b;
			if($id)	{
				$sql_del = mysql_query("delete from tbl_bannerhome  where a_id ='$id'");
			}
		}
	}
	if($com=="sort"){
	for($i=1;$i<=$socheck;$i++){
		...
		$sort_v=$$a_id2;
		if($sort_v){
		  $sql_qr = mysql_query("update tbl_bannerhome set a_sort='$sort_v' where (a_id ='$a_id1')");
		}
	}
}
	if ($del){
		$sql_del = mysql_query("delete from tbl_bannerhome  where a_id ='$del'");	
	}
	if ($ina) $sql_del = mysql_query("update tbl_bannerhome set a_of=0 where a_id ='$ina'");
	if ($a) $sql_del = mysql_query("update tbl_bannerhome set a_of=1 where a_id ='$a'");
	...
	$qr="select * from  tbl_bannerhome  $str_action $str_order_by ";
...
echo "<form action='mana_bannerhome.php' method='post' name='mainform' id='mainform'>
	  ...
	  $stt=0;
		while($rows = mysql_fetch_array($sql_list)){
			...
			$a_of  = $rows[a_of];	
			if ($a_of){ 
				$ac="<a href='mana_bannerhome.php?ina=$a_id&bs=$bs&pz=$pz' class='green'>Hiển thị >>> Ẩn</a>";		
			}else{ 
				$ac="<a href='mana_bannerhome.php?a=$a_id&bs=$bs&pz=$pz' class='gray'>Ẩn >>> Hiển thị</a>";
			}
			
			$a_sort  = $rows[a_sort];	
			$c_id = $rows[c_id];$c_name=$array_loaibanner[$c_id];
			$str_cid="<li>Danh mục<right>$c_name</right></li>";
echo "<table>
<tr>
<th width='2%'>Check</th>
<th width='7%'>S.xếp</th>
...
<td><ul>
	...
	<li>Chức năng<right><a href='edit_bannerhome.php?edit=$a_id&bs=$bs&pz=$pz' class='orange'>Sửa</a><a href='$PHP_SELF?del=$a_id&bs=$bs&pz=$pz' class='orange' onclick=\"return confirm('Bạn chắc chắn muốn xóa ?')\">Xóa</a>$ac</right></li>
</ul></td>
</tr>
</table><br>";}
...
if($numshow > 1){
	...
	if($cs>0){
		for($i = 1; $i <= $cs; $i++){
			...
			$kt=$i*$gioihancuanum;
			$tam=$bt-1; $str_pagelist.="<a href='mana_bannerhome.php?bs=$bs&pz=$tam' class='blue'>[$bt-$kt]</a> ";
		}
	}
	if($pz){$tam=$pz-1; $str_pagelist.="<a href='mana_bannerhome.php?bs=$bs&pz=$tam' class='gray'>« trước</a>";}
	if ($batdau <= $ketthuc){
		for($i = $batdau; $i < $ketthuc; $i++){
			$k = $i+1;
			if($i==$pz) $str_pagelist.="<a class='green'>$k</a>";
			else{$tam=$i; $str_pagelist.="<a href='mana_bannerhome.php?bs=$bs&pz=$tam' class='blue'>$k</a>";}
		}
	}
	if(!($pz>=$numshow-1)) { $tam=$pz+1;$str_pagelist.="<a href='mana_bannerhome.php?bs=$bs&pz=$tam' class='gray'>tiếp »</a>";}
	...
	if($cs+1<=$numshow1){
		for($i = $cs+1; $i <= $numshow1; $i++){
			...
			$tam=$bt-1;
			if($bt<=$kt) $str_pagelist.="<a href='mana_bannerhome.php?bs=$bs&pz=$tam' class='blue'>[$strbtkt]</a> ";
		}
	}
}
