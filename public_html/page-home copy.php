...
?>
    ...
    <div id="homeBottom">
        <div class="title">
            ...
            <a href="<?=$sub_root?>/nhan-su-chu-luc" class="viewmore"><?=$lb_xemthem?></a>
        </div>
        <div id="team">
		<?
	$sql_list = mysql_query("select * from  tbl_news where (a_of=1) and (c_kind='5') order by  a_id  desc limit 0,4");
	while($rows = mysql_fetch_array($sql_list)){
		$a_pic = $rows[a_pic];$a_name_no = outputdata($rows[a_name_no]);
		...
		echo "<li class='onePer'>
                <h1><a href='$sub_root/nhan-su-chu-luc/$a_name_no'>$a_name</a></h1>
