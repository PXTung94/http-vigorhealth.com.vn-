...
<?
$sql_listcata = mysql_query("select  * from tbl_pagename_ctrinh where (page_parent=0) and (page_of=1) and (page_name_no='$spageid')");
...
if($spageidsub){
	$sql_listcata = mysql_query("select  * from tbl_pagename_ctrinh where (page_parent='$page_id') and (page_of=1) and (page_name_no='$spageidsub')");
	$page_id = trim(@mysql_result($sql_listcata,0,'page_id'));$page_name_no = trim(@mysql_result($sql_listcata,0,'page_name_no'));
	if($se_lang=="eng"){ $page_name = trim(@mysql_result($sql_listcata,0,'page_name_en'));}else{  $page_name = trim(@mysql_result($sql_listcata,0,'page_name'));}
}
	...
	$kind=$page_id;$content_statics="";$title_download="";$file_download="";
	$qr=mysql_query("select * from tbl_otherpage_ctrinh where okind='$kind'");
	if (@mysql_num_rows($qr)){
		...
		$sql_picogiua = mysql_query("select * from tbl_pic_otherpage_ctrinh where n_id='$id_id' and kind=$kind order by n_vitri");
...
<div id="colRight">
	<div class="pageNav">
    	<ul>
            ...
            <li>&rsaquo;</li>
    		<li><a href="#"><?=$lb_gioithieu?></a></li>
            <li>&rsaquo;</li>
    		<li><a href="<?=$sub_root?>/gioi-thieu/<?=$page_name_no?>"><?=$page_name?></a></li>
    	</ul>
        <div class="clr"></div>
